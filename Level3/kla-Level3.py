...
curr_y = 0
count = 0
for i in range(input['die']['rows']):
  image = []
  for j in range(input['die']['columns']):
    image.append(imageGray[count])
    count +=1
  if i%2==0:
    imagesResized.append(image)
  else:
    imagesResized.append(image.reverse())

  
print(len(imagesResized[0][0][0]))
imageGray = imagesResized
data = []
# # print(img1.shape)
...
for k in range(noImg):
  for l in range(len(input['care_areas'])):
    for i in range(input['die']['height']-1-input['care_areas'][l]['top_left']['y'],input['die']['height']-input['care_areas'][l]['bottom_right']['y']):
      for j in range(input['care_areas'][l]['top_left']['x'],input['care_areas'][l]['bottom_right']['x']):
        row = []
